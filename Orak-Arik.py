...
cap = cv2.VideoCapture("Video/Smart Parking.mp4")  # For Video
log = {}
slot_status = {}
prev_slot_status = {}
        ...
        for car_box in car_boxes:
            ...
            if car_x < x + width and car_x + car_w > x and car_y < y + height and car_y + car_h > y:
                ...
                if not (car_id in log):
                    log_value = {car_id:[(i+1), datetime.datetime.now()]}
...
while True:
    ...
    for k, v in slot_status.items():
        print(k,v)
    print("="*20)
    for key, value in sorted(log.items(), key=lambda x:x[1]):
        ID = str(key).split(".")[0]
        waktu = str(datetime.datetime.now()-value[1]).split(".")[0]

        cv2.putText(img, f"ID {ID}", (int(852-170), int(40+(n*20))), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1)
        cv2.putText(img, f"{value[0]}", (int(852-125), int(40+(n*20))), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100,0,255), 1)
        cv2.putText(img, f"{waktu}", (int(852-100), int(40+(n*20))), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1)
        n += 1
